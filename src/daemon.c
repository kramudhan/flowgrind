...
/** To indicate flowgrind daemons already have UUID. */
int have_uuid = 0;
/** UUID (by time) flowgrind daemons. */
char server_uuid[38];
		...
		case REQUEST_GET_STATUS:
			{
				...
				/* Get the UUID(or get a new one) from the daemon */
 ...
 * for reporting interval. The daemon maintain all its data in its flow
 ...
 *  destination) and report type (interval or final) are used to identify the 
 *  flow report in controller.
 *
 * @param[in] flow flow structure maintained by a daemon
 ...
 *
 * The daemon maintain all its data in its flow statistics data structure.
 ...
 *
 * @param[in] uuid_str daemons UUID
 */
void get_uuid_string(char *uuid_str)
{
	...
	if (!have_uuid){
		if((uuid_generate_time_safe(uuid)) != 0){
			logging_log(LOG_WARNING, "failed to generate UUID in safe manner");
		}
