		   ...
		   void * const user_data)
{
	...
	int rc;
	xmlrpc_value *ret = 0;
	struct request_get_uuid *request = 0;

	DEBUG_MSG(LOG_WARNING, "Method get_uuid called");

	request = malloc(sizeof(struct request_get_uuid));
	rc = dispatch_request((struct request*)request, REQUEST_GET_UUID);

	if (rc == -1)
		XMLRPC_FAIL(env, XMLRPC_INTERNAL_ERROR, request->r.error); /* goto cleanup on failure */

	/* Return our result. */
	ret = xmlrpc_build_value(env, "{s:s}",
