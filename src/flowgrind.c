...
static void find_daemon(xmlrpc_client *rpc_client)
{
	...
	while (node) {
		...
		xmlrpc_client_call2f(&rpc_env, rpc_client, flow_rpc_info->server_url, 
				"get_uuid", &resultP, "()");
		die_if_fault_occurred(&rpc_env);
		...
		if (resultP) {
			...
			for (unsigned id = 0; id < copt.num_flows; id++) {
				foreach(int *i, SOURCE, DESTINATION) {
					struct flow_endpoint* endpoint = &cflow[id].endpoint[*i];
					if(!strcmp(endpoint->rpc_info->server_url, flow_rpc_info->server_url) 
							&& !endpoint->daemon) {
						endpoint->daemon = 
							set_unique_daemon_by_uuid(server_uuid, 
